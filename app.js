...
function renderCompletionProfile(category) {
  ...
  ui.completionProfileBadge.textContent = (category.shortLabel || category.title || "?").slice(0, 2).toUpperCase();
}
...
function createMissionCard(mission) {
  ...
  card.innerHTML = `
    <div class="mission-row__icon" aria-hidden="true">${badgeText}</div>
...
function renderHomeProfile(category) {
  ...
  ui.homeProfileBadge.textContent = (category.shortLabel || category.title || "?").slice(0, 2).toUpperCase();
}
