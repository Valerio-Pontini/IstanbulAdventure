window.APP_TEXT = {
  "it": {
    "content": {
      "app": {
        "coverTitle": "Istanbul Adventure",
        "coverHint": "Tocca per entrare",
        "contentTitle": "Benvenuto",
        "actionButtonLabel": "Missione 0",
        "storySlides": [
          "Questa non è una guida, è un modo diverso di scoprire la città.",
          "Durante il viaggio non seguirai un percorso fisso. Scoprirai Istanbul attraverso missioni, dettagli e storie.",
          "A Istanbul non tutti vedono la stessa città: ogni viaggiatore appartiene, senza saperlo, a una delle cinque tribù",
          "Prima di iniziare, una breve missione servirà a capire a quale tribù appartieni: un quiz veloce con domande a volte un po’ assurde, che definirà il tuo modo di giocare e inizierà a farti scoprire Istanbul in modo unico."
        ]
      },
      "result": {
        "kicker": "Esito Missione 0",
        "titleEyebrow": "Missione chiusa",
        "title": "Il viaggio ha trovato il tuo sguardo",
        "text": "Da qui in poi non esplorerai Istanbul in modo generico. Il tuo profilo ha sbloccato una direzione narrativa e una selezione di missioni coerenti con il modo in cui osservi la città.",
        "cardNote": "Puoi sostituire testi e immagini in questo file senza toccare la logica dell'app.",
        "homeButtonLabel": "Apri la home",
        "homeRoute": "#home",
        "illustration": {
          "src": "./assets/ui/welcome-bg.jpg",
          "alt": "Scenario di Istanbul"
        },
        "emblem": {
          "src": "./assets/symbols/generic.svg",
          "alt": "Sigillo finale"
        }
      },
      "home": {
        "statusLabel": "Archivio missioni attivo",
        "hero": {
          "kicker": "Home missioni",
          "title": "Le tue missioni a Istanbul",
          "text": "Qui trovi le missioni generiche, quelle legate ai luoghi e il percorso speciale sbloccato dalla tua personalità di esplorazione."
        },
        "profileCard": {
          "kicker": "Personalità emersa",
          "fallbackTitle": "Personalità in attesa",
          "fallbackText": "Completa la Missione 0 per sbloccare il tuo percorso personale."
        },
        "overview": {
          "kicker": "Mappa operativa",
          "title": "Scegli da dove iniziare",
          "text": "La home mostra una selezione consigliata. Ogni categoria ha poi un archivio dedicato con filtri rapidi.",
          "stats": {
            "totalLabel": "Missioni consigliate",
            "generalLabel": "Generiche",
            "locationLabel": "Luoghi",
            "personalLabel": "Per te"
          }
        },
        "navigation": {
          "title": "Apri un archivio missioni"
        },
        "detailView": {
          "backLabel": "Torna alla home",
          "filterTitle": "Filtra le missioni",
          "emptyTitle": "Nessuna missione con questi filtri",
          "emptyText": "Prova a cambiare filtro per vedere altre missioni."
        },
        "missionDetail": {
          "backLabel": "Torna all'elenco",
          "tagsLabel": "Tag di lettura",
          "objectiveTitle": "Obiettivo e contesto",
          "flowTitle": "Flusso di completamento",
          "flowSingle": "step",
          "summaryType": "Tipo",
          "summaryAudience": "Accesso",
          "summaryFormat": "Formato",
          "summaryPlace": "Luogo",
          "summaryCompletion": "Verifica"
        },
        "sections": {
          "general": {
            "anchorLabel": "Generiche",
            "kicker": "Missioni generiche",
            "title": "Azioni che puoi fare quasi ovunque",
            "text": "Missioni leggere e immediate, pensate per accompagnare il viaggio.",
            "detailTitle": "Archivio missioni generiche"
          },
          "locations": {
            "anchorLabel": "Luoghi",
            "kicker": "Missioni luogo",
            "title": "Missioni legate a punti specifici",
            "text": "Obiettivi che si accendono quando arrivi in luoghi simbolici della città.",
            "detailTitle": "Archivio missioni per luogo"
          },
          "personal": {
            "anchorLabel": "Per te",
            "kicker": "Missioni personalità",
            "title": "Missioni calibrate sulla personalità della tua tribù",
            "text": "Il quiz ha individuato il tuo modo di leggere Istanbul. Qui trovi le missioni più coerenti con quella personalità.",
            "detailTitle": "Archivio missioni personali"
          }
        },
        "categories": {
          "a": {
            "title": "Archivisti degli Imperi",
            "shortLabel": "Imperi",
            "description": "Cerchi stratificazioni storiche, architetture di potere e tracce lasciate dalle epoche della città.",
            "iconSrc": "./assets/symbols/archivisti.svg",
            "iconAlt": "Icona Archivisti degli Imperi"
          },
          "c": {
            "title": "Cacciatori di Leggende",
            "shortLabel": "Leggende",
            "description": "Segui racconti urbani, superstizioni, misteri e dettagli che sembrano nascondere una storia.",
            "iconSrc": "./assets/symbols/leggende.svg",
            "iconAlt": "Icona Cacciatori di Leggende"
          },
          "e": {
            "title": "Esploratori della Città",
            "shortLabel": "Città",
            "description": "Ti guidano traiettorie, traghetti, panorami, quartieri e connessioni tra i lati di Istanbul.",
            "iconSrc": "./assets/symbols/citta.svg",
            "iconAlt": "Icona Esploratori della Città"
          },
          "cu": {
            "title": "Custodi delle Tradizioni",
            "shortLabel": "Tradizioni",
            "description": "Leggi Istanbul attraverso çay, cibo, rituali sociali, mercati e abitudini quotidiane.",
            "iconSrc": "./assets/symbols/tradizioni.svg",
            "iconAlt": "Icona Custodi delle Tradizioni"
          },
          "d": {
            "title": "Decifratori dei Simboli",
            "shortLabel": "Simboli",
            "description": "Noti nazar, tulipani, mosaici, pattern e piccoli segni che quasi tutti ignorano.",
            "iconSrc": "./assets/symbols/simboli.svg",
            "iconAlt": "Icona Decifratori dei Simboli"
          }
        },
        "missions": {
          "catalog": {
            "general_tre_segnali_in_10_minuti": {
              "title": "Tre segnali in 10 minuti",
              "description": "Raccogli tre dettagli che cambiano il modo in cui percepisci la strada in cui ti trovi.",
              "meta": "Ovunque / 10 min",
              "recommended": true,
              "filters": [
                "breve",
                "osservazione",
                "ovunque"
              ],
              "iconSrc": "./assets/symbols/generic.svg",
              "iconAlt": "Icona missione generica"
            },
            "general_audio_invisibile": {
              "title": "Audio invisibile",
              "description": "Fermati un minuto e annota tre suoni che definiscono il luogo più di qualsiasi foto.",
              "meta": "Ovunque / ascolto",
              "recommended": true,
              "filters": [
                "breve",
                "ascolto",
                "ovunque"
              ],
              "iconSrc": "./assets/symbols/generic.svg",
              "iconAlt": "Icona missione generica"
            },
            "general_cartolina_futura": {
              "title": "Cartolina futura",
              "description": "Scegli un punto della città e descrivilo come se dovessi consigliarlo a qualcuno tra dieci anni.",
              "meta": "Ovunque / scrittura",
              "filters": [
                "riflessione",
                "ovunque"
              ],
              "iconSrc": "./assets/symbols/generic.svg",
              "iconAlt": "Icona missione generica"
            },
            "locations_bosforo_scegli_una_soglia": {
              "title": "Bosforo: scegli una soglia",
              "description": "Sul traghetto o sulla riva, individua il punto in cui la città sembra cambiare identità.",
              "meta": "Bosforo",
              "recommended": true,
              "filters": [
                "bosforo",
                "movimento",
                "luogo"
              ],
              "iconSrc": "./assets/symbols/luoghi.svg",
              "iconAlt": "Icona missione luogo"
            },
            "locations_grand_bazaar_mappa_una_deviazione": {
              "title": "Grand Bazaar: mappa una deviazione",
              "description": "Entra in un corridoio secondario e registra cosa cambia rispetto alla rotta principale.",
              "meta": "Grand Bazaar",
              "recommended": true,
              "filters": [
                "bazaar",
                "mappatura",
                "luogo"
              ],
              "iconSrc": "./assets/symbols/luoghi.svg",
              "iconAlt": "Icona missione luogo"
            },
            "locations_cisterna_basilica_cerca_il_fuori_tono": {
              "title": "Cisterna Basilica: cerca il fuori tono",
              "description": "Trova un elemento che sembra appartenere a un'altra epoca o a un'altra storia.",
              "meta": "Cisterna Basilica",
              "filters": [
                "cisterna",
                "osservazione",
                "luogo"
              ],
              "iconSrc": "./assets/symbols/luoghi.svg",
              "iconAlt": "Icona missione luogo"
            },
            "locations_moschea_blu_leggi_il_ritmo": {
              "title": "Moschea Blu: leggi il ritmo",
              "description": "Resta fermo qualche minuto e osserva come il luogo cambia con ingressi, uscite e soste.",
              "meta": "Moschea Blu",
              "filters": [
                "moschea",
                "ritmo",
                "luogo"
              ],
              "iconSrc": "./assets/symbols/luoghi.svg",
              "iconAlt": "Icona missione luogo"
            },
            "a_ricostruisci_una_linea_del_tempo": {
              "title": "Ricostruisci una linea del tempo",
              "description": "Scegli un luogo e annota tre epoche che, in modi diversi, lo attraversano ancora oggi.",
              "meta": "Archivisti degli Imperi",
              "recommended": true,
              "filters": [
                "storia",
                "stratificazione",
                "profilo"
              ],
              "iconSrc": "./assets/symbols/archivisti.svg",
              "iconAlt": "Icona missione Archivisti"
            },
            "a_caccia_al_riuso": {
              "title": "Caccia al riuso",
              "description": "Individua un frammento architettonico o simbolico riutilizzato in un contesto nuovo.",
              "meta": "Archivisti degli Imperi",
              "filters": [
                "architettura",
                "riuso",
                "profilo"
              ],
              "iconSrc": "./assets/symbols/archivisti.svg",
              "iconAlt": "Icona missione Archivisti"
            },
            "c_leggenda_locale": {
              "title": "Leggenda locale",
              "description": "Trova un racconto, una superstizione o un aneddoto e trascrivilo in una frase sola.",
              "meta": "Cacciatori di Leggende",
              "recommended": true,
              "filters": [
                "leggende",
                "racconto",
                "profilo"
              ],
              "iconSrc": "./assets/symbols/leggende.svg",
              "iconAlt": "Icona missione Leggende"
            },
            "c_oggetto_con_doppia_vita": {
              "title": "Oggetto con doppia vita",
              "description": "Scegli un oggetto visto oggi e immagina la storia nascosta che potrebbe portarsi dietro.",
              "meta": "Cacciatori di Leggende",
              "filters": [
                "immaginazione",
                "mistero",
                "profilo"
              ],
              "iconSrc": "./assets/symbols/leggende.svg",
              "iconAlt": "Icona missione Leggende"
            },
            "e_deviazione_utile": {
              "title": "Deviazione utile",
              "description": "Abbandona il percorso più ovvio e trova una scorciatoia che cambi il ritmo dell'esplorazione.",
              "meta": "Esploratori della Città",
              "recommended": true,
              "filters": [
                "percorso",
                "movimento",
                "profilo"
              ],
              "iconSrc": "./assets/symbols/citta.svg",
              "iconAlt": "Icona missione Città"
            },
            "e_nodo_di_passaggio": {
              "title": "Nodo di passaggio",
              "description": "Identifica un punto dove flussi, direzioni e intenzioni diverse si incrociano.",
              "meta": "Esploratori della Città",
              "filters": [
                "mappa",
                "quartieri",
                "profilo"
              ],
              "iconSrc": "./assets/symbols/citta.svg",
              "iconAlt": "Icona missione Città"
            },
            "cu_una_frase_da_conservare": {
              "title": "Una frase da conservare",
              "description": "Porta via una micro-conversazione, un saluto o una frase che valga come souvenir umano.",
              "meta": "Custodi delle Tradizioni",
              "recommended": true,
              "filters": [
                "persone",
                "rituali",
                "profilo"
              ],
              "iconSrc": "./assets/symbols/tradizioni.svg",
              "iconAlt": "Icona missione Tradizioni"
            },
            "cu_geografia_delle_persone": {
              "title": "Geografia delle persone",
              "description": "Osserva come cambia l'energia di un luogo quando cambiano i gruppi che lo attraversano.",
              "meta": "Custodi delle Tradizioni",
              "filters": [
                "sociale",
                "osservazione",
                "profilo"
              ],
              "iconSrc": "./assets/symbols/tradizioni.svg",
              "iconAlt": "Icona missione Tradizioni"
            },
            "d_archivio_dei_micro_dettagli": {
              "title": "Archivio dei micro-dettagli",
              "description": "Fotografa o annota tre dettagli che raccontano il luogo meglio della vista d'insieme.",
              "meta": "Decifratori dei Simboli",
              "recommended": true,
              "filters": [
                "dettaglio",
                "simboli",
                "profilo"
              ],
              "iconSrc": "./assets/symbols/simboli.svg",
              "iconAlt": "Icona missione Simboli"
            },
            "d_pattern_ricorrente": {
              "title": "Pattern ricorrente",
              "description": "Segui un colore, un simbolo o una texture e scopri dove riappare nella giornata.",
              "meta": "Decifratori dei Simboli",
              "filters": [
                "pattern",
                "mosaici",
                "profilo"
              ],
              "iconSrc": "./assets/symbols/simboli.svg",
              "iconAlt": "Icona missione Simboli"
            }
          },
          "sections": {
            "general": [
              "general_tre_segnali_in_10_minuti",
              "general_audio_invisibile",
              "general_cartolina_futura"
            ],
            "locations": [
              "locations_bosforo_scegli_una_soglia",
              "locations_grand_bazaar_mappa_una_deviazione",
              "locations_cisterna_basilica_cerca_il_fuori_tono",
              "locations_moschea_blu_leggi_il_ritmo"
            ],
            "personalByCategory": {
              "a": [
                "a_ricostruisci_una_linea_del_tempo",
                "a_caccia_al_riuso"
              ],
              "c": [
                "c_leggenda_locale",
                "c_oggetto_con_doppia_vita"
              ],
              "e": [
                "e_deviazione_utile",
                "e_nodo_di_passaggio"
              ],
              "cu": [
                "cu_una_frase_da_conservare",
                "cu_geografia_delle_persone"
              ],
              "d": [
                "d_archivio_dei_micro_dettagli",
                "d_pattern_ricorrente"
              ]
            }
          }
        }
      },
      "quiz": {
        "introTitle": "Missione 0",
        "introText": "Scegli ogni risposta d'istinto.",
        "tapToContinueLabel": "Tocca per continuare",
        "completion": {
          "title": "Percorso avviato",
          "text": "La tua tribù sta prendendo forma. Da qui in poi il viaggio potrà seguire strade diverse in base alle tue scelte."
        },
        "questionsById": {
          "p1": {
            "prompt": "Sei sul Bosforo, il traghetto passa vicino a una moschea enorme",
            "answers": [
              {
                "label": "Pensi a quante epoche ha attraversato",
                "nextId": "a_1"
              },
              {
                "label": "Ti chiedi se ha storie strane legate a lei",
                "nextId": "c_1"
              },
              {
                "label": "Guardi il percorso per arrivarci",
                "nextId": "e_1"
              },
              {
                "label": "Noti i dettagli della struttura e delle decorazioni",
                "nextId": "d_1"
              }
            ]
          },
          "p2": {
            "prompt": "Nel Gran Bazar un venditore ti invita a entrare",
            "answers": [
              {
                "label": "Inizi a parlare con lui",
                "nextId": "cu_4"
              },
              {
                "label": "Osservi gli oggetti esposti",
                "nextId": "d_4"
              },
              {
                "label": "Ti chiedi che tipo di storie avrà visto",
                "nextId": "c_4"
              },
              {
                "label": "Ti infili dentro per vedere dove arrivi",
                "nextId": "e_4"
              }
            ]
          },
          "p3": {
            "prompt": "Ti trovi davanti a un edificio antico senza spiegazioni",
            "answers": [
              {
                "label": "Ti avvicini per osservare i dettagli",
                "nextId": "d_7"
              },
              {
                "label": "Lo superi per trovare qualcos'altro",
                "nextId": "e_7"
              },
              {
                "label": "Cerchi di capire quando è stato scotruito",
                "nextId": "a_7"
              },
              {
                "label": "Immagini cosa potrebbe essere successo lì",
                "nextId": "c_7"
              }
            ]
          },
          "p4": {
            "prompt": "In una çay house qualcuno ti guarda e sorride",
            "answers": [
              {
                "label": "Ti chiedi chi sia e cosa stia pensando",
                "nextId": "c_10"
              },
              {
                "label": "Inizi la conversazione",
                "nextId": "cu_10"
              },
              {
                "label": "Noti piccoli gesti e oggetti sul tavolo",
                "nextId": "d_10"
              },
              {
                "label": "Ti godi la scena e vivi il momento",
                "nextId": "e_10"
              }
            ]
          },
          "p5": {
            "prompt": "Camminando trovi una strada che sembra poco frequentata",
            "answers": [
              {
                "label": "Ti chiedi dove porta",
                "nextId": "e_13"
              },
              {
                "label": "Ti chiedi cosa succede lì di notte",
                "nextId": "c_13"
              },
              {
                "label": "Guardi se qualche simbolo o indicazione dice dove porta",
                "nextId": "d_13"
              },
              {
                "label": "Ti chiedi cosa è successo lì",
                "nextId": "a_13"
              }
            ]
          },
          "p6": {
            "prompt": "Ti danno un bicchiere di çay",
            "answers": [
              {
                "label": "Pensi alla tradizione dietro alla preparazione",
                "nextId": "c_16"
              },
              {
                "label": "Lo bevi e riparti subito",
                "nextId": "e_16"
              },
              {
                "label": "Osservi il modo in cui viene servito",
                "nextId": "d_16"
              },
              {
                "label": "Ti chiedi chi lo abbia inventato",
                "nextId": "cu_16"
              }
            ]
          },
          "p7": {
            "prompt": "Vedi un simbolo blu appeso ovunque",
            "answers": [
              {
                "label": "Chiedi a qualcuno cosa significa",
                "nextId": "cu_19"
              },
              {
                "label": "Ti chiedi che storia c'è dietro",
                "nextId": "c_19"
              },
              {
                "label": "Lo osservi nei dettagli",
                "nextId": "d_19"
              },
              {
                "label": "Ti chiedi da quando esiste",
                "nextId": "a_19"
              }
            ]
          },
          "p8": {
            "prompt": "Stai scegliendo cosa fotografare",
            "answers": [
              {
                "label": "Una strada",
                "nextId": "e_22"
              },
              {
                "label": "Un luogo con una storia",
                "nextId": "a_22"
              },
              {
                "label": "Un momento tra persone",
                "nextId": "cu_22"
              },
              {
                "label": "Un dettaglio",
                "nextId": "d_22"
              }
            ]
          },
          "p9": {
            "prompt": "Qualcuno racconta una leggenda su un luogo",
            "answers": [
              {
                "label": "Ti interessa capire se è vera",
                "nextId": "a_25"
              },
              {
                "label": "Ti interessa la storia in sè",
                "nextId": "c_25"
              },
              {
                "label": "Ti interessa andare a vedere il posto",
                "nextId": "e_25"
              },
              {
                "label": "Ti interessa cosa rappresenta",
                "nextId": "d_25"
              }
            ]
          },
          "p10": {
            "prompt": "Fine giornata, ripensi a quello che hai visto:",
            "answers": [
              {
                "label": "Le persone incontrate",
                "nextId": "cu_28"
              },
              {
                "label": "Le storie strane",
                "nextId": "c_28"
              },
              {
                "label": "I percorsi fatti",
                "nextId": "e_28"
              },
              {
                "label": "I dettagli notati",
                "nextId": "d_28"
              }
            ]
          },
          "a_1": {
            "prompt": "Istanbul è stata capitale di quanti imperi?",
            "feedback": "È stata capitale dell'impero romano, bizantino e ottomano",
            "nextId": "a_2",
            "answers": [
              "1",
              "2",
              "3",
              "4"
            ]
          },
          "a_2": {
            "prompt": "Prima di chiamarsi Istanbul come si chiamava?",
            "feedback": "la città nacque come colonia greca chiamata Bisanzio nel VII secolo a.C. Nel 330 d.C. l'imperatore Costantino I la rifondò come nuova capitale dell'Impero Romano chiamandola Costantinopoli. Costantinopoli fu il centro dell'impero Bizantino per oltre 1000 anni. Il nome Istanbul divenne ufficilae nel 1930 nonostante fosse già usato informalmente dalla popolazione da secoli ",
            "nextId": "a_3",
            "answers": [
              "Izmir",
              "Costantinopoli",
              "Efeso",
              "Bisanzio"
            ]
          },
          "a_3": {
            "prompt": "Chi conquistò Costantinopoli nel 1453?",
            "feedback": "Mehmet II, detto 'Il Conquistatore', aveva solo 21 anni",
            "nextId": "p2",
            "answers": [
              "Bayezid I",
              "Mehmet II",
              "Murad II",
              "gedik Ahmet"
            ]
          },
          "a_7": {
            "prompt": "Quale edificio è stato sia chiesa che moschea?",
            "feedback": "Hagia Sophia è uno dei simboli più incredibili della città. Costruita nel 537 d.C. sotto l'imperatode Giustiniano I fu dapprima chiesa fino al 1453, poi divenne moschea fino al 1934. Dal 1935 Santa Sofia è diventata un vero e proprio museo, per tornare ad essere una moschea solo nel 2020.",
            "nextId": "a_8",
            "answers": [
              "Topkapi",
              "Hagia Sophia",
              "Moschea Blu",
              "Dolmabahçe"
            ]
          },
          "a_8": {
            "prompt": "Il Gran Bazar nasce",
            "feedback": "Nel 1400, è uno dei mercati coperti più antichi del mondo",
            "nextId": "a_9",
            "answers": [
              "Nel 1800",
              "Nel 1200",
              "Nel 1400",
              "Nel 1900"
            ]
          },
          "a_9": {
            "prompt": "Il palazzo Topkapi era",
            "feedback": "La residenza dei sultani: fu il centro del potere ottomano per secoli",
            "nextId": "p4",
            "answers": [
              "Una fortezza militare",
              "La residenza dei sultani",
              "Un tempio",
              "Un mercato"
            ]
          },
          "a_13": {
            "prompt": "Il nome Istanbul probabilmente deriva da:",
            "feedback": "Deriva dall'espressione greca 'èis tèn pòlin' che significa 'verso la città'.",
            "nextId": "a_14",
            "answers": [
              "Latino",
              "Greco",
              "Arabo",
              "Persiano"
            ]
          },
          "a_14": {
            "prompt": "Hagia Sophia è stata costruita in:",
            "feedback": "Fu incredibilmente costruita in 5 anni",
            "nextId": "a_15",
            "answers": [
              "10 anni",
              "50 anni",
              "5 anni",
              "100 anni"
            ]
          },
          "a_15": {
            "prompt": "I sultani ottomani vivevano:",
            "feedback": "In un complesso enorme con centinaia di stanze: Topkapi era praticamente una città dentro la città",
            "nextId": "p6",
            "answers": [
              "Sempre nello stesso palazzo",
              "In tende",
              "In un complesso enorme con centinaia di stanze",
              "In castelli separati"
            ]
          },
          "a_19": {
            "prompt": "Le mura di Costantinopoli avevano:",
            "feedback": "Tre livelli difensivi: fossa, mura esterne e interne",
            "nextId": "a_20",
            "answers": [
              "Una sola linea difensiva",
              "Due linee difensive",
              "Tre linee difensive",
              "Nessuna linea difensiva"
            ]
          },
          "a_20": {
            "prompt": "Durante l'assedio del 1453, gli ottomani:",
            "feedback": "Trasportarono navi su terra per aggirare le difese",
            "nextId": "a_21",
            "answers": [
              "Scavarono sotto la città",
              "Portarono navi via terra",
              "Usarono solo archi",
              "Aspettarono"
            ]
          },
          "a_21": {
            "prompt": "Hagia Sophia ha:",
            "feedback": "Entrambi, un mix unico di simboli religiosi",
            "nextId": "p8",
            "answers": [
              "Solo mosaici cristiani",
              "Solo elementi islamici",
              "Entrambi",
              "Nessuno"
            ]
          },
          "a_22": {
            "prompt": "La Cisterna Basilica era:",
            "feedback": "Serviva a immagazzinare acqua per la città",
            "nextId": "a_23",
            "answers": [
              "Una prigione",
              "Una riserva d'acqa",
              "Un tempio",
              "Un teatro"
            ]
          },
          "a_23": {
            "prompt": "Le teste di Medusa nella cisterna sono:",
            "feedback": "Capovolte. Posizione misteriosa e molto discussa",
            "nextId": "a_24",
            "answers": [
              "Colorate",
              "Distrutte",
              "Capovolte",
              "Divise"
            ]
          },
          "a_24": {
            "prompt": "I Giannizzeri erano",
            "feedback": "Bambini presi da altre regioni. Venivano reclutati da giovani e addestrati per servire il sultano",
            "nextId": "p9",
            "answers": [
              "Soldati di famiglie nobili",
              "Bambini presi da altre regioni",
              "Mercenari europei",
              "Prigionieri"
            ]
          },
          "a_25": {
            "prompt": "Alcuni mosaici di Hagia Sophia sono stati:",
            "feedback": "Furono coperti durante il periodo ottomano e riscoperti dopo",
            "nextId": "a_26",
            "answers": [
              "Distrutti",
              "Coperti per secoli",
              "Rubati",
              "Dipinti"
            ]
          },
          "a_26": {
            "prompt": "Il Tünel di Istanbul è considerato:",
            "feedback": "è il secondo sistema sotterraneo urbano più antico dopo Londra",
            "nextId": "a_27",
            "answers": [
              "La seconda seggiovia al mondo",
              "La seconda metro al mondo",
              "Il primo ascensore al mondo",
              "La prima funivia moderna"
            ]
          },
          "a_27": {
            "prompt": "Le cucine del palazzo Topkapi:",
            "feedback": "Preparavano migliaia di pasti al giorno. Una vera macchina logistica imperiale",
            "nextId": "p10",
            "answers": [
              "Servivano solo il sultano",
              "Preparavano migliaia di pasti al giorno",
              "Erano il nascondiglio del tesoro reale",
              "Non furono mai utilizzate"
            ]
          },
          "c_1": {
            "prompt": "Secondo una leggenda, la Torre della Fanciulla serviva a:",
            "feedback": "Tenere al sicuro una principessa. Fu isolata lì per sfuggire a una profezia.",
            "nextId": "c_2",
            "answers": [
              "Difendere la città",
              "Tenere al sicuro una principessa",
              "Nascondere un tesoro",
              "Controllare il traffico marittimo"
            ]
          },
          "c_2": {
            "prompt": "Nella leggenda della Torre della Fanciulla, cosa uccide la principessa?",
            "feedback": "Un serpente nascosto. Arrivò in un cesto di frutta.",
            "nextId": "c_3",
            "answers": [
              "Un soldato",
              "Un serpente nascosto",
              "Un crollo",
              "Una malattia"
            ]
          },
          "c_3": {
            "prompt": "Secondo alcune credenze, il malocchio viene causato da:",
            "feedback": "Lo sguardo invidioso. Il nazar serve a proteggersi da questo.",
            "nextId": "p2",
            "answers": [
              "Il vento",
              "Lo sguardo invidioso",
              "Gli animali",
              "Il buio"
            ]
          },
          "c_4": {
            "prompt": "Secondo alcune storie, i gatti di Istanbul sono:",
            "feedback": "Custodi della città. Sono visti come osservatori silenziosi.",
            "nextId": "c_5",
            "answers": [
              "Animali normali",
              "Custodi della città",
              "Animali sacri",
              "Animali randagi senza ruolo"
            ]
          },
          "c_5": {
            "prompt": "Quale rito popolare si associa comunemente alla colonna nella Moschea Blu di Istanbul?",
            "feedback": "Il rito consiste nell'inserire il pollice in un piccolo foro della colonna e ruotarlo esprimendo un desiderio.",
            "nextId": "c_6",
            "answers": [
              "Inserire il pollice nel foro e girarlo",
              "Baciare la colonna di pietra",
              "Lucidare la colonna con olio sacro",
              "Portare offerte di fiori"
            ]
          },
          "c_6": {
            "prompt": "Qual è l’unica fonte letteraria nota che racconta il volo di Hezarfen?",
            "feedback": "Il volo è descritto solo nel Seyahatnâme di Evliya Çelebi. Secondo la leggenda Hezarfen si lanciò dalla Torre di galata usando ali artificiali da lui stesso costruite, attraversò il Bosforo planando ed atterrò nel quartiere di Üsküdar, sulla sponda asiatica.",
            "answers": [
              {
                "label": "Il Seyahatnâme di Evliya Çelebi",
                "nextId": "p3"
              },
              {
                "label": "Un salmo bizantino",
                "nextId": "p3"
              },
              {
                "label": "Un diario ottomano",
                "nextId": "p3"
              },
              {
                "label": "Una cronaca persiana",
                "nextId": "p3"
              }
            ]
          },
          "c_7": {
            "prompt": "Qual è il soprannome turco della Cisterna Basilica?",
            "feedback": "È comunemente chiamata Yerebatan Sarayı, che significa 'Palazzo sommerso'.",
            "answers": [
              {
                "label": "Palazzo sommerso",
                "nextId": "c_8"
              },
              {
                "label": "Sotterraneo Regale",
                "nextId": "c_8"
              },
              {
                "label": "Abisso di Giustiniano",
                "nextId": "c_8"
              },
              {
                "label": "Sala delle Meduse",
                "nextId": "c_8"
              }
            ]
          },
          "c_8": {
            "prompt": "Come viene rappresentata Şahmeran nella mitologia turca?",
            "feedback": "Şahmeran è descritta come un essere mitologico metà donna e metà serpente ed è simbolo di saggezza, conoscen<a e mistero.",
            "answers": [
              {
                "label": "Metà donna, metà serpente",
                "nextId": "c_9"
              },
              {
                "label": "Un drago alato",
                "nextId": "c_9"
              },
              {
                "label": "Una ninfa dei boschi",
                "nextId": "c_9"
              },
              {
                "label": "Una statua vivente",
                "nextId": "c_9"
              }
            ]
          },
          "c_9": {
            "prompt": "Nasreddin Hoca è celebre per racconti di quale genere?",
            "feedback": "Nasreddin è noto per racconti umoristici che capovolgono la logica comune.",
            "answers": [
              {
                "label": "Umorismo paradossale",
                "nextId": "p4"
              },
              {
                "label": "Tragedie storiche",
                "nextId": "p4"
              },
              {
                "label": "Scienza e invenzioni",
                "nextId": "p4"
              },
              {
                "label": "Ricette culinarie",
                "nextId": "p4"
              }
            ]
          },
          "c_10": {
            "prompt": "Chi è Dede Korkut nelle saghe turche?",
            "feedback": "Dede Korkut è il saggio narratore e figura guida nell’epica popolare degli Oghuz.",
            "answers": [
              {
                "label": "Saggio narratore e consigliere epico",
                "nextId": "c_11"
              },
              {
                "label": "Re guerriero degli imperatori",
                "nextId": "c_11"
              },
              {
                "label": "Demone della foresta",
                "nextId": "c_11"
              },
              {
                "label": "Drago marino",
                "nextId": "c_11"
              }
            ]
          },
          "c_11": {
            "prompt": "Chi sono Karagöz e Hacivat?",
            "feedback": "Karagöz e Hacivat sono i protagonisti del tradizionale teatro di ombre turco.",
            "answers": [
              {
                "label": "Personaggi del teatro di ombre turco",
                "nextId": "c_12"
              },
              {
                "label": "Due moschee a Istanbul",
                "nextId": "c_12"
              },
              {
                "label": "Due re della dinastia ottomana",
                "nextId": "c_12"
              },
              {
                "label": "Due montagne sacre",
                "nextId": "c_12"
              }
            ]
          },
          "c_12": {
            "prompt": "Quale sultano ottomano introdusse il caffè in Turchia secondo la leggenda?",
            "feedback": "Secondo la tradizione, il caffè entrò in uso all’epoca di Solimano il Magnifico nel XVI secolo.",
            "answers": [
              {
                "label": "Solimano il Magnifico",
                "nextId": "p5"
              },
              {
                "label": "Mehmed II",
                "nextId": "p5"
              },
              {
                "label": "Suleyman il Vecchio",
                "nextId": "p5"
              },
              {
                "label": "Mahmud II",
                "nextId": "p5"
              }
            ]
          },
          "c_13": {
            "prompt": "Cosa racconta la leggenda sulla resa dei turchi a Vienna nel 1683 legata al caffè?",
            "feedback": "La leggenda narra che i turchi, sconfitti a Vienna, abbandonarono sacchi di caffè. In questo modo il caffè entrò in Europa.",
            "answers": [
              {
                "label": "Abbandonarono sacchi di caffè",
                "nextId": "c_14"
              },
              {
                "label": "Pagarono un riscatto con caffè speziato",
                "nextId": "c_14"
              },
              {
                "label": "Ai vinti fu imposta una tariffa sul caffè",
                "nextId": "c_14"
              },
              {
                "label": "I turchi rubarono tutto il caffè di Vienna",
                "nextId": "c_14"
              }
            ]
          },
          "c_14": {
            "prompt": "Chi è Asena",
            "feedback": "Asena è una figura centrale nei miti delle origini dei popoli turchi. Secondo la leggenda, una lupa trova un bambino ferito (unico sopravvissuto di una tribù distrutta), lo salva e lo alleva. Da questo bambino nascerà una discendenza che darà origine a al popolo turco.",
            "answers": [
              {
                "label": "Una dea del mare che protegge i marinai",
                "nextId": "c_15"
              },
              {
                "label": "Una lupa che salva e dà origine al popolo turco",
                "nextId": "c_15"
              },
              {
                "label": "Una principessa guerriera trasformata in aquila",
                "nextId": "c_15"
              },
              {
                "label": "Una strega che vieve nelle montagne",
                "nextId": "c_15"
              }
            ]
          },
          "c_15": {
            "prompt": "Cosa significa il nome Şahmeran nella lingua persiana?",
            "feedback": "Il nome Şahmeran significa letteralmente \"Re dei serpenti\"(persiano shah: re, maran: serpenti).",
            "answers": [
              {
                "label": "Re dei serpenti",
                "nextId": "p6"
              },
              {
                "label": "Signora delle acque",
                "nextId": "p6"
              },
              {
                "label": "Regina delle fate",
                "nextId": "p6"
              },
              {
                "label": "Dea della saggezza",
                "nextId": "p6"
              }
            ]
          },
          "c_16": {
            "prompt": "Secondo la leggenda, quale segno compare sul corpo di chi vede Şahmeran?",
            "feedback": "Si narra che chi vede Şahmeran sviluppa delle squame serpentine sul corpo.",
            "answers": [
              {
                "label": "Squame serpentine",
                "nextId": "c_17"
              },
              {
                "label": "Ali d'angelo",
                "nextId": "c_17"
              },
              {
                "label": "Occhi di fuoco",
                "nextId": "c_17"
              },
              {
                "label": "Nessun segno",
                "nextId": "c_17"
              }
            ]
          },
          "c_17": {
            "prompt": "Secondo il mito di Ergenekon, in quale luogo leggendario i Turchi furono imprigionati?",
            "feedback": "Il mito narra che gli antichi Turchi furono rinchiusi nella valle di Ergenekon fino alla loro liberazione.",
            "answers": [
              {
                "label": "Valle di Ergenekon",
                "nextId": "c_18"
              },
              {
                "label": "Grotta di Catalhoyuk",
                "nextId": "c_18"
              },
              {
                "label": "Isola di Creta",
                "nextId": "c_18"
              },
              {
                "label": "Deserto di Gobi",
                "nextId": "c_18"
              }
            ]
          },
          "c_18": {
            "prompt": "Cos’è l’Anka kuşu nella mitologia turca?",
            "feedback": "L’Anka kuşu è la figura della fenice nella mitologia turca, un uccello leggendario che rinasce dalle proprie ceneri.",
            "answers": [
              {
                "label": "La fenice mitica",
                "nextId": "p7"
              },
              {
                "label": "Una montagna sacra",
                "nextId": "p7"
              },
              {
                "label": "Un eroe epico",
                "nextId": "p7"
              },
              {
                "label": "Una pianta medicinale",
                "nextId": "p7"
              }
            ]
          },
          "c_19": {
            "prompt": "Quale figura leggendaria è strettamente associata alle acque e alla guarigione nella tradizione popolare turca?",
            "feedback": "Il profeta leggendario Khidr Hızır è spesso associato a sorgenti sacre e guarigione nella tradizione popolare turca.",
            "answers": [
              {
                "label": "Khidr Hızır",
                "nextId": "c_20"
              },
              {
                "label": "Orfeo",
                "nextId": "c_20"
              },
              {
                "label": "Ülgen",
                "nextId": "c_20"
              },
              {
                "label": "Erlik",
                "nextId": "c_20"
              }
            ]
          },
          "c_20": {
            "prompt": "Nel mito di Ergenekon, come fu liberato il popolo dalla valle leggendaria?",
            "feedback": "La leggenda dice che un fabbro forgiò una spada e ruppe la montagna di Ergenekon, liberando così il popolo.",
            "answers": [
              {
                "label": "Un fabbro forgiò una spada e aprì la montagna",
                "nextId": "c_21"
              },
              {
                "label": "Un angelo discese dal cielo",
                "nextId": "c_21"
              },
              {
                "label": "Un drago sorvolò la valle",
                "nextId": "c_21"
              },
              {
                "label": "Un re spezzò la valle con il suo scettro",
                "nextId": "c_21"
              }
            ]
          },
          "c_21": {
            "prompt": "Quale figura leggendaria appare nella notte di Hıdrellez secondo la tradizione?",
            "feedback": "Durante Hıdrellez si crede che appaia Hızır, figura misteriosa associata a fortuna, guarigione e desideri realizzati.",
            "answers": [
              {
                "label": "Hızır",
                "nextId": "p8"
              },
              {
                "label": "Basat",
                "nextId": "p8"
              },
              {
                "label": "Topegöz",
                "nextId": "p8"
              },
              {
                "label": "Anubi",
                "nextId": "p8"
              }
            ]
          },
          "c_25": {
            "prompt": "Secondo alcune leggende, cosa succede di notte nella Cisterna Basilica?",
            "feedback": "Molte storie raccontano che nella Cisterna Basilica si sentano voci e sussurri, alimentando l’atmosfera misteriosa del luogo sotterraneo.",
            "answers": [
              {
                "label": "Si sentono voci e sussurri",
                "nextId": "c_26"
              },
              {
                "label": "L'acqua scompare",
                "nextId": "c_26"
              },
              {
                "label": "Le statue si muovono",
                "nextId": "c_26"
              },
              {
                "label": "Appare un re",
                "nextId": "c_26"
              }
            ]
          },
          "c_26": {
            "prompt": "Chi era Erlik nella tradizione anatolica?",
            "feedback": "Erlik è la divinità che governa il mondo sotterraneo nella mitologia turca. È associato alla morte, all’oscurità e al giudizio delle anime. Spesso è visto come l’opposto di Ülgen, il dio del cielo, creando un equilibrio tra bene e male nell’universo mitologico.",
            "answers": [
              {
                "label": "Il dio del cielo e della luce",
                "nextId": "c_27"
              },
              {
                "label": "Il signore degli inferi e delle forze oscure",
                "nextId": "c_27"
              },
              {
                "label": "Un eroe guerriero delle steppe",
                "nextId": "c_27"
              },
              {
                "label": "Uno spirito protettore degli animali",
                "nextId": "c_27"
              }
            ]
          },
          "c_27": {
            "prompt": "Quale figura mitologica turca rappresenta un uccello immortale che rinasce?",
            "feedback": "L’Anka è un uccello mitologico simile alla fenice, simbolo di rinascita e immortalità nella tradizione turca.",
            "answers": [
              {
                "label": "Anka",
                "nextId": "p10"
              },
              {
                "label": "Simurgh",
                "nextId": "p10"
              },
              {
                "label": "Grifone",
                "nextId": "p10"
              },
              {
                "label": "Pegaso",
                "nextId": "p10"
              }
            ]
          },
          "cu_4": {
            "prompt": "Qual è la bevanda tradizionale turca servita in piccoli bicchieri a forma di tulipano?",
            "feedback": "Il tè (çay) è la bevanda nazionale turca, servita in bicchieri a tulipano ed elemento centrale della vita sociale.",
            "nextId": "cu_5",
            "answers": [
              "Caffè turco",
              "Tè turco (çay)",
              "Ayran",
              "Salep"
            ]
          },
          "cu_5": {
            "prompt": "Il caffè turco tradizionale viene preparato in un recipiente chiamato:",
            "feedback": "Il caffè turco si prepara nel cezve (detto anche ibrik), piccolo bricco con manico lungo.",
            "nextId": "cu_6",
            "answers": [
              "Ibrik",
              "Cezve",
              "Samovar",
              "Tandir"
            ]
          },
          "cu_6": {
            "prompt": "Quale dolce ottomano è composto da sottilissimi strati di pasta fillo, pistacchi e sciroppo?",
            "feedback": "La baklava è un dolce ottomano a strati di pasta fillo, frutta secca e sciroppo.",
            "nextId": "p3",
            "answers": [
              "Lokum",
              "Baklava",
              "Kadayif",
              "Simit"
            ]
          },
          "cu_16": {
            "prompt": "Secondo un proverbio turco, per quanti anni resta memorabile un caffè offerto?",
            "feedback": "Quaranta anni. Il proverbio turco recita «bir fincan kahvenin kırk yıl hatırı vardır», cioè 'un caffè ha memoria per 40 anni'.",
            "nextId": "cu_17",
            "answers": [
              "1",
              "74",
              "40",
              "111"
            ]
          },
          "cu_17": {
            "prompt": "Quale tradizione turca è riconosciuta Patrimonio Immateriale UNESCO dal 2013?",
            "feedback": "Il rituale del caffè turco. La preparazione e l’offerta del caffè secondo la tradizione ottomana sono state inserite nel 2013 fra i Patrimoni Culturali Immateriali dell'UNESCO.",
            "nextId": "cu_18",
            "answers": [
              "Il rito del caffè turco",
              "Lotta con l’olio",
              "Anadolu Rock",
              "Pittura Ebru"
            ]
          },
          "cu_18": {
            "prompt": "Cos'è il 'simit' in Turchia?",
            "feedback": "Una ciambella al sesamo. Il simit è un pane a forma di ciambella ricoperto di semi di sesamo, molto diffuso come spuntino in Turchia.",
            "nextId": "p7",
            "answers": [
              "Un formaggio",
              "Una ciambella al sesamo",
              "Un dolce al miele",
              "Una zuppa fredda"
            ]
          },
          "cu_19": {
            "prompt": "Come si chiamano i ravioli turchi ripieni di carne?",
            "feedback": "Manti. Sono ravioli di piccole dimensioni ripieni di carne macinata, di solito serviti con yogurt e salsa piccante.",
            "nextId": "cu_20",
            "answers": [
              "Dolma",
              "Börek",
              "Manti",
              "Köfte"
            ]
          },
          "cu_20": {
            "prompt": "Quando si entra in una casa turca, cosa viene spesso offerto agli ospiti?",
            "feedback": "Pantofole. In Turchia è usanza offrire pantofole agli ospiti quando entrano in casa, così da togliersi le scarpe e sentirsi a proprio agio.",
            "nextId": "cu_21",
            "answers": [
              "Caffè turco",
              "Pantofole",
              "Dolci",
              "Cappellini"
            ]
          },
          "cu_21": {
            "prompt": "Cosa succede durante l'Altın Günü turco?",
            "feedback": "Donne si scambiano monete d'oro. L'Altın Günü è un incontro mensile fra donne, in cui ognuna porta una moneta d'oro da donare alla padrona di casa.",
            "nextId": "p8",
            "answers": [
              "Vengono scambiati buoi",
              "Donne si scambiano monete d'oro",
              "Si accendono falò",
              "Si raccontano storie di gatti"
            ]
          },
          "cu_22": {
            "prompt": "Durante il Ramadan, a che cosa servono i tamburini notturni?",
            "feedback": "Svegliare i fedeli per il sahur. I tamburini suonano di notte per svegliare la gente a consumare il pasto prima dell'alba.",
            "nextId": "cu_23",
            "answers": [
              "Annunciare la mezzanotte",
              "Svegliare i fedeli per il sahur",
              "Raccogliere offerte",
              "Guidare processioni"
            ]
          },
          "cu_23": {
            "prompt": "Qual è la tradizione del caffè nella cerimonia di fidanzamento turca?",
            "feedback": "Il caffè dello sposo è salato. Nella cerimonia di fidanzamento, la futura sposa prepara al promesso sposo una tazza di caffè salato che lui deve bere come prova d'amore.",
            "nextId": "cu_24",
            "answers": [
              "Il caffè della sposa è dolce",
              "Il caffè dello sposo è salato",
              "Si beve solo tè",
              "La bevanda non viene mai servita"
            ]
          },
          "cu_24": {
            "prompt": "Che sport tradizionale turco consiste in lotte tra atleti coperti d'olio?",
            "feedback": "Lotta col grasso (Yağlı güreş). È la lotta tradizionale in cui i contendenti si cospargono di olio e indossano pantaloni di pelle, cercando di buttarsi giù a terra.",
            "nextId": "p9",
            "answers": [
              "Wrestling greco-romano",
              "Lotta col grasso",
              "Sumo turco",
              "Lotta con i cammelli"
            ]
          },
          "cu_28": {
            "prompt": "Nella tradizione popolare turca, cosa simboleggia una stella cadente?",
            "feedback": "La morte. Nel folclore turco, vedere una stella cadente è considerato un segnale funesto, presagio di morte imminente.",
            "nextId": "cu_29",
            "answers": [
              "La morte",
              "Un desiderio realizzato",
              "Buona fortuna",
              "La fine del mondo"
            ]
          },
          "d_1": {
            "prompt": "A cosa serve il nazar boncuğu nella cultura turca?",
            "feedback": "Proteggere dal malocchio. Il nazar boncuğu, l'occhio blu, è un amuleto tradizionale usato per tenere lontano il malocchio e la sfortuna.",
            "nextId": "d_2",
            "answers": [
              "Predire il futuro",
              "Proteggere dal malocchio",
              "Misurare la pioggia",
              "Richiamare la fortuna"
            ]
          },
          "d_2": {
            "prompt": "Quale fiore è simbolo nazionale della Turchia, celebrato con festival storici?",
            "feedback": "Tulipano. Il tulipano è stato coltivato in Turchia sin dall'epoca ottomana e dedicato a celebrazioni primaverili come simbolo nazionale.",
            "nextId": "d_3",
            "answers": [
              "Rosa",
              "Tulipano",
              "Girasole",
              "Loto"
            ]
          },
          "d_3": {
            "prompt": "Lasciare una forbice aperta in casa porta quale conseguenza, secondo le credenze turche?",
            "feedback": "Una lite in famiglia. Si dice che una forbice lasciata aperta in casa porterà inevitabilmente a una disputa tra i membri della famiglia.",
            "nextId": "p2",
            "answers": [
              "Una lite in famiglia",
              "Una pioggia improvvisa",
              "Buona salute",
              "Felicità eterna"
            ]
          },
          "d_4": {
            "prompt": "Cosa presagisce se gli escrementi di un uccello cadono sulla testa di qualcuno, nella cultura turca?",
            "feedback": "Buona fortuna. Secondo la tradizione, se un uccello lascia cadere un escremento sulla testa di una persona, quel giorno sarà fortunato e ricco.",
            "nextId": "d_5",
            "answers": [
              "Buona fortuna",
              "Malattia",
              "Perdita del portafoglio",
              "Maledizione"
            ]
          },
          "d_5": {
            "prompt": "Secondo un detto, come porta fortuna entrare in una stanza in Turchia?",
            "feedback": "Con il piede destro. Tradizionalmente si crede che entrare in una stanza con il piede destro porti buona sorte.",
            "nextId": "d_6",
            "answers": [
              "Con il piede destro",
              "In punta di piedi",
              "Di spalle",
              "Gridando"
            ]
          },
          "d_6": {
            "prompt": "Qual è la differenza principale tra l'Adana kebab e l'Urfa kebab?",
            "feedback": "Aglio e peperoncino. L'Adana kebab tradizionale contiene aglio e una dose maggiore di peperoncino rispetto all'Urfa kebab.",
            "nextId": "p3",
            "answers": [
              "Aglio e peperoncino",
              "Formaggio e peperoncino",
              "Nessuna differenza",
              "Solo aglio"
            ]
          },
          "d_7": {
            "prompt": "Cosa raffigura la bandiera turca?",
            "feedback": "Una mezzaluna e una stella. La bandiera della Turchia è rossa con una mezzaluna crescente e una stella bianca, simboli storicamente associati all'Islam.",
            "nextId": "d_8",
            "answers": [
              "Una mezzaluna e una stella",
              "Tre strisce orizzontali",
              "Un leone rampante",
              "Un'aquila reale"
            ]
          },
          "d_8": {
            "prompt": "Come si chiama la festa che segna la fine del Ramadan in Turchia?",
            "feedback": "Şeker Bayramı (festa dello zucchero). Questa festa celebra la fine del Ramadan e si festeggia mangiando dolci e passando tempo con la famiglia.",
            "nextId": "d_9",
            "answers": [
              "Kurban Bayramı",
              "Nevruz",
              "Şeker Bayramı",
              "Hıdırellez"
            ]
          },
          "d_9": {
            "prompt": "Cosa si dice che avvenga se si vede una cicogna volare, secondo il folclore turco?",
            "feedback": "Si passerà l'anno a viaggiare. Una credenza popolare afferma che chi vede una cicogna in volo trascorrerà l'anno viaggiando.",
            "nextId": "p4",
            "answers": [
              "Si passerà l'anno a viaggiare",
              "Si avrò un figlio",
              "Troverà dell'oro",
              "Nessun evento particolare"
            ]
          },
          "d_10": {
            "prompt": "Quale tipica bevanda invernale turca, a base di cereali fermentati, è densa e dolce?",
            "feedback": "Boza. La boza è una bevanda invernale densa e dolce ottenuta dalla fermentazione di cereali, tradizionale nel nord-est e nei Balcani.",
            "nextId": "d_11",
            "answers": [
              "Ayran",
              "Raki",
              "Boza",
              "Salep"
            ]
          },
          "d_11": {
            "prompt": "Il termine 'lokum' deriva da una parola che significa:",
            "feedback": "Conforto per la gola. La parola 'lokum' proviene dall'ottomano-arabo rahat al-hulqûm, che significa letteralmente 'conforto per la gola'.",
            "nextId": "d_12",
            "answers": [
              "Conforto per la gola",
              "Dolcezza infinita",
              "Tesoro del sultano",
              "Ricchezza del mare"
            ]
          },
          "d_12": {
            "prompt": "Secondo una leggenda persiana, dalle lacrime di quale personaggio innamorato nacquero i tulipani rossi?",
            "feedback": "Ferhad, che pianse la morte di Shirin. Le sue lacrime di amore e dolore si trasformarono in tulipani rossi.",
            "nextId": "p5",
            "answers": [
              "Ferhad",
              "Shirin",
              "Suleiman il Magnifico",
              "Un sultano sconosciuto"
            ]
          },
          "d_13": {
            "prompt": "Secondo una tradizione turca, l'asure (dolce tradizionale) sarebbe stato l'ultimo pasto consumato a bordo di quale famosa imbarcazione?",
            "feedback": "L'Arca di Noè. La leggenda vuole che l'asure fosse preparato da Noè come pasto finale dopo il Diluvio.",
            "nextId": "d_14",
            "answers": [
              "L'Arca di Noè",
              "Nave di Alessandro Magno",
              "Santa Maria di Colombo",
              "Una caravella vichinga"
            ]
          },
          "d_14": {
            "prompt": "Come fu costruita in una notte, secondo la leggenda, la chiesa di Santo Stefano dei Bulgari a Istanbul?",
            "feedback": "Con pezzi prefabbricati di ghisa assemblati rapidamente. Il sultano concesse la chiesa a condizione che fosse eretta in una notte, e fu trasportata e montata da componenti di ghisa.",
            "nextId": "d_15",
            "answers": [
              "Con pezzi prefabbricati di ghisa assemblati rapidamente",
              "Con incantesimi magici",
              "Fu invece costruita in dieci anni",
              "Con mano invisibile degli ingegneri"
            ]
          },
          "d_15": {
            "prompt": "Nei mosaici bizantini di Istanbul, l’oro simboleggia:",
            "feedback": "La luce divina. Nei mosaici bizantini l’oro rappresenta il sacro e l’eterno.",
            "nextId": "p6",
            "answers": [
              "Ricchezza materiale",
              "La luce divina",
              "Il potere politico",
              "Il sole reale"
            ]
          },
          "d_16": {
            "prompt": "La calligrafia islamica è considerata:",
            "feedback": "Un’arte sacra. È una delle forme artistiche più importanti nel mondo islamico.",
            "nextId": "d_17",
            "answers": [
              "Decorazione casuale",
              "Un’arte sacra",
              "Solo scrittura funzionale",
              "Arte moderna"
            ]
          },
          "d_17": {
            "prompt": "Nei tappeti turchi, i motivi geometrici spesso rappresentano:",
            "feedback": "Concetti simbolici come protezione e infinito. I tappeti raccontano storie attraverso i motivi.",
            "nextId": "d_18",
            "answers": [
              "Solo estetica",
              "Concetti simbolici",
              "Mappe geografiche",
              "Scene di caccia"
            ]
          },
          "d_18": {
            "prompt": "Il simbolo della mano di Fatima (Hamsa) è usato per:",
            "feedback": "Protezione dal malocchio. È un simbolo diffuso nel Mediterraneo.",
            "nextId": "p7",
            "answers": [
              "Decorazione artistica",
              "Protezione dal malocchio",
              "Indicare potere politico",
              "Segnalare ricchezza"
            ]
          },
          "d_19": {
            "prompt": "Durante il Ramadan, tra i minareti possono comparire:",
            "feedback": "Scritte luminose. Le mahya sono frasi illuminate tra i minareti delle moschee.",
            "nextId": "d_20",
            "answers": [
              "Bandierine decorative",
              "Scritte luminose",
              "Specchi riflettenti",
              "Campane decorative"
            ]
          },
          "d_20": {
            "prompt": "La colonna serpentina di Istanbul era associata a:",
            "feedback": "Protezione dai serpenti. Era considerata un simbolo apotropaico nella tradizione popolare.",
            "nextId": "d_21",
            "answers": [
              "Protezione dai serpenti",
              "Calendari astronomici",
              "Depositi d’acqua",
              "Navigazione"
            ]
          },
          "d_21": {
            "prompt": "La calligrafia islamica nelle moschee serve principalmente a:",
            "feedback": "Trasmettere testi religiosi. La calligrafia sostituisce spesso le immagini figurative.",
            "nextId": "p8",
            "answers": [
              "Decorare",
              "Trasmettere testi religiosi",
              "Indicare uscite",
              "Firmare gli artisti"
            ]
          },
          "d_22": {
            "prompt": "Le lanterne tradizionali nei bazar sono simbolo di:",
            "feedback": "Atmosfera e tradizione artigianale. Sono elementi culturali oltre che funzionali.",
            "nextId": "d_23",
            "answers": [
              "Illuminazione moderna",
              "Atmosfera e tradizione artigianale",
              "Segnaletica urbana",
              "Sistema di sicurezza"
            ]
          },
          "d_23": {
            "prompt": "Il gesto di togliersi le scarpe prima di entrare in certi luoghi indica:",
            "feedback": "Rispetto e purificazione. È una pratica legata a contesti religiosi e domestici.",
            "nextId": "d_24",
            "answers": [
              "Tradizione sportiva",
              "Rispetto e purificazione",
              "Controllo e sicurezza",
              "Sottomissione"
            ]
          },
          "d_24": {
            "prompt": "Le porte decorate nelle case tradizionali indicavano spesso:",
            "feedback": "Status e identità familiare. Le decorazioni avevano significato sociale.",
            "nextId": "p9",
            "answers": [
              "Gusto estetico",
              "Status e identità familiare",
              "Prezzi delle case",
              "Percorsi urbani"
            ]
          },
          "d_25": {
            "prompt": "Il gesto di versare il tè dall’alto ha una funzione simbolica di:",
            "feedback": "Cura e attenzione verso l’ospite. È parte del rituale del servizio del tè.",
            "nextId": "d_26",
            "answers": [
              "Raffreddamento rapido",
              "Cura e attenzione verso l’ospite",
              "Tradizione militare",
              "Pulizia"
            ]
          },
          "d_26": {
            "prompt": "Il gesto di mettere la mano sul cuore quando si saluta indica:",
            "feedback": "Rispetto sincero. È un gesto di saluto molto diffuso.",
            "nextId": "d_27",
            "answers": [
              "Formalità burocratica",
              "Rispetto",
              "Segnale religioso obbligatorio",
              "Richiesta di aiuto"
            ]
          },
          "d_27": {
            "prompt": "Le piastrelle decorative (Iznik) sono famose per:",
            "feedback": "Motivi complessi e simbolici. Sono elementi iconici dell’arte ottomana.",
            "nextId": "p10",
            "answers": [
              "Colori casuali",
              "Motivi complessi e simbolici",
              "Uso industriale",
              "Segnaletica urbana"
            ]
          },
          "e_1": {
            "prompt": "Quanti sono i colli storici di Istanbul, come nella tradizione antica?",
            "feedback": "Sette colli. Istanbul è nota come la città dai sette colli.",
            "nextId": "e_2",
            "answers": [
              "5",
              "7",
              "9",
              "11"
            ]
          },
          "e_2": {
            "prompt": "Circa quante moschee si trovano a Istanbul?",
            "feedback": "Oltre 3.000 moschee. Istanbul ospita più di 3.000 moschee.",
            "nextId": "e_3",
            "answers": [
              "500",
              "1.500",
              "3.000",
              "6.000"
            ]
          },
          "e_3": {
            "prompt": "Quale celebre mezzo di trasporto storico è ancora operativo sulla via Istiklal di Istanbul?",
            "feedback": "Tram storico rosso. Sulla via Istiklal di Istanbul circola ancora il famoso tram storico di colore rosso, vero simbolo locale.",
            "nextId": "p2",
            "answers": [
              "Tram rosso",
              "Autobus a due piani",
              "Treno a vapore",
              "Carro trainati da buoi"
            ]
          },
          "e_4": {
            "prompt": "Quanti paesi confinano via terra con la Turchia?",
            "feedback": "8 paesi. La Turchia confina con 8 stati: Grecia, Bulgaria, Georgia, Armenia, Azerbaigian, Iran, Iraq e Siria.",
            "nextId": "e_5",
            "answers": [
              "6",
              "8",
              "10",
              "12"
            ]
          },
          "e_5": {
            "prompt": "Qual è la montagna più alta della Turchia?",
            "feedback": "Monte Ararat. Con 5.165 m è la vetta più alta della Turchia.",
            "nextId": "e_6",
            "answers": [
              "Monte Ararat",
              "Monte Erciyes",
              "Monte Uludağ",
              "Monte Hasan"
            ]
          },
          "e_6": {
            "prompt": "Il quartiere di Sultanahmet è famoso perché:",
            "feedback": "È il cuore storico: al suo interno ci sono Hagia Sophia, Moschea Blu e antica Costantinopoli.",
            "nextId": "p3",
            "answers": [
              "È il centro storico",
              "È moderno e finanziario",
              "È pieno di grattacieli",
              "È un porto industriale"
            ]
          },
          "e_7": {
            "prompt": "Beyoğlu (zona Galata) oggi è conosciuto soprattutto per:",
            "feedback": "È una zona vivace: locali, arte, musica e strade iconiche.",
            "nextId": "e_8",
            "answers": [
              "Vita notturna e cultura",
              "Moschee storiche",
              "Campi agricoli",
              "Miniere sotterranee"
            ]
          },
          "e_8": {
            "prompt": "Il quartiere di Balat è famoso per:",
            "feedback": "Case colorate e storia multiculturale. Balat è uno dei quartieri più fotografati, pieno di storia e colori.",
            "nextId": "e_9",
            "answers": [
              "Case colorate e storia multiculturale",
              "Grattacieli moderni",
              "Solo hotel di lusso",
              "Aeroporti"
            ]
          },
          "e_9": {
            "prompt": "Il Corno d’Oro è:",
            "feedback": "Un'insenatura naturale. È una baia che separa la parte storica da quella più moderna.",
            "nextId": "p4",
            "answers": [
              "Un'insenatura naturale",
              "Un ponte",
              "Un mercato",
              "Una moschea"
            ]
          },
          "e_10": {
            "prompt": "Le Isole dei Principi sono:",
            "feedback": "Isole senza traffico urbano. Sono famose per l’assenza di auto e per la tranquillità.",
            "nextId": "e_11",
            "answers": [
              "Isole senza traffico urbano",
              "Quartieri industriali",
              "Aeroporti marini",
              "Porti militari"
            ]
          },
          "e_11": {
            "prompt": "Il Marmaray è:",
            "feedback": "Un tunnel ferroviario sotto il Bosforo. Collega Europa e Asia via ferrovia sotto il mare.",
            "nextId": "e_12",
            "answers": [
              "Un tunnel ferroviario sotto il Bosforo",
              "Un ponte sospeso",
              "Una nave",
              "Un quartiere"
            ]
          },
          "e_12": {
            "prompt": "Il Bosforo nel punto più stretto misura circa:",
            "feedback": "Nel punto più stretto è largo circa 700 metri.",
            "nextId": "p5",
            "answers": [
              "700 metri",
              "50 metri",
              "5 km",
              "20 km"
            ]
          },
          "e_13": {
            "prompt": "Il ponte sul Bosforo inaugurato nel 1973 si chiama:",
            "feedback": "Ponte dei Martiri del 15 Luglio. È stato il primo ponte tra Europa e Asia.",
            "nextId": "e_14",
            "answers": [
              "Ponte dei Martiri del 15 Luglio",
              "Ponte Galata",
              "Golden Bridge",
              "Ponte Marmara"
            ]
          },
          "e_14": {
            "prompt": "Istanbul ha circa quanti abitanti?",
            "feedback": "15 milioni. È una delle città più popolose al mondo.",
            "nextId": "e_15",
            "answers": [
              "15 milioni",
              "2 milioni",
              "5 milioni",
              "50 milioni"
            ]
          },
          "e_15": {
            "prompt": "Il Grand Bazaar contiene circa:",
            "feedback": "4000 negozi. È uno dei mercati coperti più grandi al mondo.",
            "nextId": "p6",
            "answers": [
              "4000 negozi",
              "100 negozi",
              "500 negozi",
              "10000 negozi"
            ]
          },
          "e_16": {
            "prompt": "Quanti ponti ha Istanbul sul Bosforo?",
            "feedback": "Ci sono tre grandi ponti sospesi che collegano i continenti.",
            "nextId": "e_17",
            "answers": [
              "3",
              "1",
              "10",
              "5"
            ]
          },
          "e_17": {
            "prompt": "Sul ponte di Galata sotto il livello stradale si trovano:",
            "feedback": "Ristoranti e locali. Il ponte ha due livelli, con ristoranti sotto.",
            "nextId": "e_18",
            "answers": [
              "Ristoranti e locali",
              "Stazioni metro",
              "Magazzini militari",
              "Negozi chiusi"
            ]
          },
          "e_18": {
            "prompt": "Il 'dolmuş' è:",
            "feedback": "Un minibus condiviso. È un mezzo di trasporto pubblico molto usato. Segue percorsi fissi ma è possibile salire e scendere quasi ovunque lungo il tragitto. Il mezzo parte quando è (quasi) pieno, infatti \"dolmuş\" significa \"riempito\"",
            "nextId": "p7",
            "answers": [
              "Un minibus condiviso",
              "Un dolce",
              "Un taxi privato",
              "Un tram"
            ]
          },
          "e_22": {
            "prompt": "Ortaköy è famoso per:",
            "feedback": "Vista sul Bosforo. È uno dei luoghi più fotografati.",
            "nextId": "e_23",
            "answers": [
              "Vista sul Bosforo",
              "Miniere",
              "Industria",
              "Aeroporti"
            ]
          },
          "e_23": {
            "prompt": "Dolmabahçe Palace è famoso per:",
            "feedback": "Stile europeo molto decorato. È molto diverso dall’architettura ottomana tradizionale.",
            "nextId": "e_24",
            "answers": [
              "Avere uno stile europeo molto decorato",
              "Essere un palazzo-moschea",
              "Essere sotterraneo",
              "Essere in legno semplice"
            ]
          },
          "e_24": {
            "prompt": "Dolmabahçe Palace contiene:",
            "feedback": "Il lampadario principale è gigantesco e in cristallo di Boemia, pesa circa 4,5 tonnellate ed ospita 750 lampadine.",
            "nextId": "p9",
            "answers": [
              "Uno dei lampadari più grandi al mondo",
              "Una torre militare",
              "Un acquedotto",
              "Una piscina olimpica"
            ]
          },
          "e_25": {
            "prompt": "In piazza Taksim si trova:",
            "feedback": "Il Monumento alla Repubblica. Celebra la nascita della Repubblica turca nel 1923.",
            "nextId": "e_26",
            "answers": [
              "Il Monumento alla Repubblica",
              "Un obelisco egizio",
              "Una torre medievale",
              "Una moschea ottomana storica"
            ]
          },
          "e_26": {
            "prompt": "Molte case tradizionali di Istanbul sono riconoscibili perché:",
            "feedback": "Hanno balconi sporgenti in legno. Sono tipiche dell’architettura ottomana residenziale.",
            "nextId": "e_27",
            "answers": [
              "Hanno balconi sporgenti in legno",
              "Sono tutte colorate",
              "Sono sotterranee",
              "Sono senza finestre"
            ]
          },
          "e_27": {
            "prompt": "L’Ippodromo di Costantinopoli oggi corrisponde a:",
            "feedback": "L’antico ippodromo si trova oggi nell’attuale piazza Sultanahmet.",
            "nextId": "p10",
            "answers": [
              "Piazza Sultanahmet",
              "Piazza Taksim",
              "Un mercato coperto",
              "Un parco moderno"
            ]
          }
        }
      }
    },
    "ui": {
      "document": {
        "title": "Istanbul Adventure",
        "description": "Istanbul Adventure, un'app narrativa mobile-first per esplorare Istanbul attraverso missioni, storie e percorsi personali."
      },
      "scenes": {
        "entry": "Ingresso",
        "story": "Prologo",
        "mission": "Missione 0",
        "complete": "Esito",
        "homeFallback": "Home missioni",
        "categoryDetail": "Archivio missioni"
      },
      "static": {
        "brand": {
          "city": "Istanbul",
          "project": "Adventure"
        },
        "entry": {
          "route": "Rotta sul Bosforo",
          "season": "Esperienza mobile",
          "title": "Una città diversa per ogni viaggiatore",
          "copy": "Una missione iniziale ti assegna una personalità di esplorazione. Da lì si aprono missioni, luoghi e percorsi speciali costruiti sul modo in cui guardi Istanbul.",
          "factFormatLabel": "Formato",
          "factFormatValue": "Flusso narrativo in tap",
          "factDurationLabel": "Durata",
          "factDurationValue": "Missione rapida + archivio personale",
          "factOutputLabel": "Output",
          "factOutputValue": "Profilo, missioni consigliate, filtri"
        },
        "story": {
          "chapter": "Prologo"
        },
        "result": {
          "profileLabel": "Profilo emerso",
          "sideLabel": "Archivio sbloccato",
          "sideText": "Da qui puoi leggere le missioni generali, quelle dei luoghi e il percorso legato al tuo profilo."
        },
        "home": {
          "route": "Archivio personale",
          "profileCategoryLabel": "Categoria"
        },
        "detail": {
          "availableTitle": "Missioni disponibili"
        }
      },
      "angular": {
        "common": {
          "continue": "Continua",
          "back": "Indietro",
          "showFilters": "Mostra filtri",
          "hideFilters": "Nascondi filtri",
          "clearFilters": "Pulisci filtri",
          "allMasculine": "Tutti",
          "allFeminine": "Tutte"
        },
        "app": {
          "install": {
            "eyebrow": "Prima di iniziare",
            "title": "Aggiungi la web app alla schermata Home",
            "lead": "Per usare l'esperienza in modalita' mobile come previsto, apri questa web app dalla Home del telefono. Ti basta farlo una volta.",
            "platformLabel": "Scegli il tuo telefono",
            "iosLabel": "iPhone / iPad",
            "androidLabel": "Android",
            "iosStep1": "Apri la web app in Safari.",
            "iosStep2": "Tocca il tasto Condividi nella barra del browser.",
            "iosStep3": "Seleziona \"Aggiungi a Home\" e conferma.",
            "iosStep4": "Da quel momento entra sempre dall'icona creata sulla Home.",
            "androidStep1": "Apri la web app in Chrome.",
            "androidStep2": "Tocca il menu con i tre puntini in alto a destra.",
            "androidStep3": "Scegli \"Aggiungi a schermata Home\" oppure \"Installa app\".",
            "androidStep4": "Conferma e poi apri sempre la web app dall'icona sulla Home.",
            "hint": "Se hai già aggiunto l'app alla Home, puoi continuare subito.",
            "button": "Continua"
          }
        },
        "landing": {
          "eyebrow": "Ingresso segreto",
          "title": "Apri Istanbul dal lato che ti osserva",
          "description": "Istanbul non si mostra a tutti allo stesso modo",
          "reopenArchive": "Riapri il tuo archivio",
          "cardEyebrow": "Rituale di accesso",
          "start": "Inizia il viaggio"
        },
        "home": {
          "heroText": "Non sei in una dashboard: sei al tavolo del companion. Scegli un filo narrativo e lasciati portare dalla città.",
          "priorityTitle": "Il filo che ti conviene ora",
          "priorityHint": "Una missione pronta da seguire senza pianificare.",
          "prioritySupport": "Inizia da qui se vuoi continuità con il tuo profilo.",
          "pathsEyebrow": "Percorsi",
          "pathsTitle": "Tre strade nella stessa città",
          "pathsDescription": "Ogni voce apre un capitolo diverso dello stesso viaggio.",
          "openIndex": "Apri l'indice completo",
          "moreTitle": "Altri sentieri",
          "moreText": "L'archivio è vasto: quando vuoi sfogliare tutto senza filtri narrativi, usa l'indice missioni.",
          "goToIndex": "Vai all'indice missioni",
          "entryGeneralTitle": "Generiche",
          "entryGeneralSubtitle": "Azioni leggere da fare quasi ovunque, come battiti nel diario.",
          "entryGeneralCta": "Entra nel capitolo",
          "entryLocationsTitle": "Luoghi",
          "entryLocationsSubtitle": "Si accendono quando arrivi in punti iconici, senza bisogno di GPS.",
          "entryLocationsCta": "Segui i luoghi",
          "entryPersonalTitle": "Per te",
          "entryPersonalSubtitle": "Missioni in sintonia con il tuo sguardo emerso dal rituale iniziale.",
          "entryPersonalCta": "Apri il filo personale",
          "saveAdded": "Missione salvata tra i preferiti.",
          "saveRemoved": "Missione rimossa dai preferiti."
        },
        "archive": {
          "eyebrow": "Archivio missioni",
          "backHome": "Torna alla home",
          "search": "Ricerca",
          "searchPlaceholder": "Titolo, tema o descrizione",
          "place": "Luogo",
          "type": "Tipologia",
          "theme": "Tema",
          "budget": "Budget",
          "resultsSuffix": "risultati",
          "emptyTitle": "Nessuna missione",
          "emptyText": "Prova con filtri diversi.",
          "fallbackTitle": "Archivio missioni",
          "saveAdded": "Missione salvata tra i preferiti.",
          "saveRemoved": "Missione rimossa dai preferiti."
        },
        "allMissions": {
          "eyebrow": "Tutte le missioni",
          "title": "Vista completa delle missioni",
          "description": "Il catalogo completo delle missioni, con filtri rapidi per capire cosa ti resta da fare, cosa hai salvato e cosa hai già chiuso.",
          "backHome": "Torna alla home",
          "searchLabel": "Cerca una missione",
          "searchPlaceholder": "Titolo, tema o luogo",
          "filterByStatus": "Filtra per stato",
          "resetFilters": "Azzera filtri",
          "visibleSuffix": "missioni visibili",
          "emptyTitle": "Nessuna missione per questo filtro",
          "emptyText": "Prova a cambiare stato o ricerca per tornare all'elenco completo.",
          "chipAll": "Tutte",
          "chipTodo": "Da fare",
          "chipSaved": "Salvate",
          "chipCompleted": "Completate",
          "saveAdded": "Missione salvata tra i preferiti.",
          "saveRemoved": "Missione rimossa dai preferiti."
        },
        "story": {
          "finish": "Entra nella Missione 0"
        },
        "quiz": {
          "eyebrow": "Missione 0",
          "title": "Segui ciò che ti chiama per primo",
          "description": "Non rispondere come in un test. Scegli come se stessi sfiorando un indizio nel margine del tuo diario.",
          "choicesAria": "Risposte alla domanda",
          "revealingEyebrow": "Rivelazione in corso",
          "revealingTitle": "Il tuo sguardo sta prendendo forma",
          "revealingDescription": "Sto aprendo il risultato della Missione 0.",
          "feedbackTitle": "La città risponde subito"
        },
        "result": {
          "archetypeEyebrow": "Archetipo rivelato",
          "cardTitle": "Da ora non guarderai Istanbul, seguirai i suoi percorsi",
          "goHome": "Vai alla tua Istanbul"
        },
        "personality": {
          "eyebrow": "Profilo emerso",
          "kicker": "Il tuo modo di leggere Istanbul",
          "openPersonalArchive": "Apri archivio personale",
          "lockedTitle": "Il tuo profilo è ancora nascosto",
          "lockedText": "Completa Missione 0 per sbloccare il percorso che meglio interpreta il tuo sguardo sulla città."
        },
        "shellHeader": {
          "brandKicker": "Secret Companion",
          "brandTitle": "Istanbul Adventure",
          "navAria": "Navigazione principale",
          "entry": "Ingresso",
          "mission0": "Missione 0",
          "home": "Home",
          "missions": "Missioni",
          "menu": "Menu",
          "currentViewPrefix": "Vista corrente: ",
          "pauseTitle": "Menu di pausa",
          "pauseLead": "Torna al hub, sfoglia l'elenco, o riprendi da qui.",
          "quickLinksAria": "Collegamenti rapidi",
          "hubMissions": "Hub missioni",
          "allMissions": "Tutte le missioni",
          "prologue": "Prologo",
          "outcome": "Esito",
          "archive": "Archivio",
          "mission": "Missione"
        },
        "shellFooter": {
          "mobileNavAria": "Navigazione mobile",
          "entryTitle": "Ingresso",
          "entrySubtitle": "Rituale",
          "quizTitle": "Quiz",
          "quizSubtitle": "Missione 0",
          "homeTitle": "Home",
          "homeSubtitle": "Percorsi",
          "missionsTitle": "Missioni",
          "missionsSubtitle": "Elenco",
          "note": "Ogni missione è un invito a guardare Istanbul con più attenzione, curiosità e meraviglia."
        },
        "missionCard": {
          "completed": "Completata",
          "inProgress": "In corso",
          "saved": "Salvata",
          "toStart": "Da iniziare",
          "save": "Salva"
        },
        "missionDetail": {
          "heroBack": "Torna al hub",
          "sceneLabel": "Scene",
          "contextButton": "Approfondisci il contesto",
          "nextScene": "Avanti nel racconto",
          "backAria": "Torna al hub",
          "contextTitle": "Perché questa missione conta",
          "close": "Chiudi",
          "notFoundTitle": "Missione non trovata",
          "notFoundText": "Il percorso che stai cercando non e' disponibile. Torna al hub e scegli un'altra missione.",
          "notFoundBack": "Torna al hub",
          "reopenMission": "Riapri missione",
          "markDone": "Segna come fatta",
          "markObjectiveDonePrefix": "Segna obiettivo",
          "markObjectiveDoneSuffix": "completato",
          "removeSaved": "Rimuovi dai salvati",
          "saveMission": "Salva missione",
          "saveAdded": "Missione salvata tra i preferiti.",
          "saveRemoved": "Missione rimossa dai salvati.",
          "missionReopened": "Missione riaperta.",
          "advanceHint": "Avanza nel racconto fino al passaggio attivo per segnarlo completato.",
          "completeActiveFirst": "Completa prima l’obiettivo attivo.",
          "objectiveDoneNext": "Obiettivo completato. Puoi passare al successivo.",
          "missionCompleted": "Missione completata.",
          "objectiveReopened": "Obiettivo riaperto."
        },
        "guidedTour": {
          "close": "Chiudi tutorial",
          "next": "Avanti",
          "finish": "Fine",
          "back": "Indietro",
          "homePathsTitle": "Parti dai tre archivi principali",
          "homePathsText": "Da qui scegli subito il tipo di esplorazione: generale, luoghi o percorso personale. Tocca una card e continui in un tap.",
          "homePathsAccent": "Hub",
          "archiveFiltersToggleTitle": "Apri o chiudi i filtri quando serve",
          "archiveFiltersToggleText": "Il pulsante filtri mantiene la pagina pulita su mobile. Aprili solo quando devi affinare la ricerca.",
          "archiveFiltersToggleAccent": "Filtro rapido",
          "archiveFiltersTitle": "Scegli i filtri con pochi tocchi",
          "archiveFiltersText": "Ricerca, luogo, tipo, tema e budget riducono l’elenco in tempo reale. Cosi trovi subito missioni fattibili nel momento giusto.",
          "archiveFiltersAccent": "Selezione",
          "archiveResultsTitle": "Controlla risultati e apri la missione",
          "archiveResultsText": "In alto vedi quante missioni restano. Poi apri la card migliore o salvala per dopo, senza perdere il contesto.",
          "archiveResultsAccent": "Risultati",
          "missionActionsTitle": "Nel dettaglio gestisci il progresso",
          "missionActionsText": "Qui fai avanzare la scena, salvi la missione o la segni completata. Tutte le azioni chiave sono a portata di pollice.",
          "missionActionsAccent": "Avanzamento"
        },
        "components": {
          "choiceLayerAria": "Scelte disponibili",
          "missionHeroEyebrow": "Scheda missione",
          "missionHeroBack": "Torna alle missioni",
          "missionFactsEyebrow": "Coordinate essenziali",
          "missionFactsTitle": "Coordinate essenziali",
          "missionStatusEyebrow": "Stato",
          "missionStatusTitle": "Azioni rapide",
          "missionStatusBack": "Torna alle missioni"
        },
        "catalog": {
          "sequentialPathLabel": "Percorso sequenziale"
        }
      },
      "actions": {
        "enterMission": "Inizia Missione 0",
        "jumpToQuiz": "Vai subito al quiz",
        "storyPrev": "Indietro",
        "storyNext": "Continua",
        "storyFinish": "Vai al quiz",
        "openHome": "Apri la home",
        "restart": "Ricomincia",
        "openArchive": "Apri archivio",
        "backToHome": "Torna alla home",
        "dismissOverlay": "Continua"
      },
      "quiz": {
        "primaryQuestionLabel": "Nodo narrativo",
        "branchQuestionLabel": "Approfondimento",
        "tapHint": "Tocca per continuare",
        "filterAll": "Tutte"
      },
      "counts": {
        "totalsSuffix": "totali",
        "missionsSuffix": "missioni",
        "resultsSuffix": "risultati"
      }
    }
  }
};
